...
public class RangeConstraint<T extends Comparable<T>> implements ISimpleConstraint<T> {
    ...
    public static <T extends Comparable<T>> Range<T> parseRangeExpression(String expr, SerializableFunction<String, T> converter) {
        ...
        if (isNotBlank(g2)) {
            newRange = ("(".equals(g1))
                ? newRange.intersection(Range.greaterThan(converter.apply(g2)))
                : newRange.intersection(Range.atLeast(converter.apply(g2)));
        }
        if (isNotBlank(g3)) {
            newRange = (")".equals(g4))
                ? newRange.intersection(Range.lessThan(converter.apply(g3)))
                : newRange.intersection(Range.atMost(converter.apply(g3)));
        }
        return newRange;
    }

    public static void main(String[] args) {
        System.out.println(parseRangeExpression("[1..10]", Integer::parseInt));
        System.out.println(parseRangeExpression("(1..10)", Integer::parseInt));
        System.out.println(parseRangeExpression("(1..)", Integer::parseInt));
        System.out.println(parseRangeExpression("(..10)", Integer::parseInt));
